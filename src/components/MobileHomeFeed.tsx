    ...
    <div className="md:hidden flex flex-col min-h-screen relative bg-on-surface">
      ...
      <header className="fixed top-0 w-full z-40 bg-gradient-to-b from-black/60 to-transparent py-4 px-6">
        <div className="flex gap-4 overflow-x-auto hide-scrollbar">
          {mobileStories.map((story, idx) => (
            <div
              key={story.id}
              className={`flex-shrink-0 w-14 h-14 rounded-full p-0.5 ${
                idx === 0 ? "border-2 border-white" : "border-2 border-primary"
              }`}
            >
              <div className="relative w-full h-full rounded-full overflow-hidden">
                <Image
                  ...
                  alt={story.username}
                  fill
                  className={`object-cover ${idx > 0 ? "opacity-80" : ""}`}
                  sizes="56px"
                />
              </div>
            </div>
      ...
      <main className="flex-1 overflow-y-scroll snap-y snap-mandatory h-screen">
        {mobileFeedPosts.map((post) => (
          <section
            key={post.id}
            className="h-screen w-full snap-start relative flex flex-col justify-end pb-24"
          >
            ...
            <div className="relative z-10 px-6 space-y-4">
              <div className="flex items-center gap-3">
                ...
                <button className="bg-white/20 backdrop-blur-md px-4 py-1.5 rounded-full text-white text-[10px] font-bold uppercase">
                  Follow
                </button>
              </div>
              ...
              {post.hashtags && (
                <div className="flex gap-2">
                  {post.hashtags.map((tag) => (
                    <span
                      key={tag}
                      className="bg-white/10 backdrop-blur-md px-3 py-1 rounded-full text-white/80 text-[10px] font-medium"
                    >
                      {tag}
                    </span>
                  ))}
      ...
      <nav className="fixed bottom-0 w-full flex justify-around items-center px-4 py-3 pb-8 bg-[#faf9f8]/90 backdrop-blur-2xl rounded-t-[2rem] z-50 shadow-[0_-8px_30px_rgb(0,0,0,0.04)]">
        <a
          className="flex flex-col items-center justify-center text-primary bg-primary/10 rounded-full px-4 py-2 active:scale-90 duration-300"
          href="#"
        >
        ...
        <a
          className="flex flex-col items-center justify-center text-[#1a1c1c]/50 active:scale-90 duration-300"
          href="#"
        >
